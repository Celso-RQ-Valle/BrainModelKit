...
from brainmodelkit.metrics.pandas import auc_gini, curve_roc


def test_curve_roc_matches_auc_and_handles_ties() -> None:
    df = pd.DataFrame({"score": [3, 2, 2, 1], "target": [1, 1, 0, 0]})
    points = curve_roc(df=df)
    assert points["fpr"].tolist() == [0, 0, 0.5, 1]
    assert points["tpr"].tolist() == [0, 0.5, 1, 1]
    assert points["threshold"].tolist() == [float("inf"), 3, 2, 1]
    area = (points.fpr.diff() * (points.tpr + points.tpr.shift()) / 2).sum()
    assert area == pytest.approx(auc_gini(df=df).iloc[0]["auc"])


def test_curve_roc_missing_and_invalid_data() -> None:
    df = pd.DataFrame({"prediction": [0.9, 0.1, None], "label": [1, 0, 1]})
    assert len(curve_roc("prediction", df, "label")) == 3
    for data in [df.iloc[:0], df.iloc[:1]]:
    ...
    with pytest.raises(ValueError, match="binary"):
        curve_roc(df=pd.DataFrame({"score": [1, 2], "target": [0, 2]}))
